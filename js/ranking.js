(function () {
  ...
  const SECTION_CONFIG = {
    ...
    diamonds: {
      value(entry) {
        const total = Math.max(0, entry.diamantes || 0);
        const suffix = total === 1 ? ' medalha diamante' : ' medalhas diamante';
        return `${numberFormatter.format(total)} ${suffix}`;
      },
      detail() {
        return '';
      }
  ...
  function initRankingCarousel(scope) {
    ...
    track.style.setProperty('--ranking-carousel-count', String(cards.length));
